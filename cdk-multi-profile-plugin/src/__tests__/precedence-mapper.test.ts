...
import {
  EnvironmentAwareGlobalProfileMapper,
  JsonFileProfileMapper,
} from '../profile-mapper';
...
describe('Global configuration', () => {
  ...
  beforeEach(() => {
    process.env[
      EnvironmentAwareGlobalProfileMapper.environmentVariableName
    ] = `${process.cwd()}/src/__tests__/global.plugin.config.json`;
    profiles = readProfiles();
  });
...
describe('When an invalid json file format is deteced', () => {
  it('should return an empty profile mapping when json file is not correctly formatted', () => {
    ...
    expect(consoleSpy).toBeCalledWith(
      'Failed to parse file README.md: ',
